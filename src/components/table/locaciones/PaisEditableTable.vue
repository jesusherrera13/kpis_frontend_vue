...
async function confirmDelete() {
    try {
        Object.assign(store.pais, itemToDelete.value);
        const response = store.delete();

        response.then(() => {
            store.fetchPaises();
            showAlert('success', 'País eliminado con éxito');
        }).catch(error => {
            showAlert('error', 'Error al eliminar el país');
        });
...
function save() {
    Object.assign(store.pais, editedItem.value);
    let response;
    if (store.pais.id) {
        response = store.update();
        showAlert('success', 'País actualizado con éxito');
    } else {
        response = store.store();
        showAlert('success', 'País guardado con éxito');
    }

    response.then(() => {
        store.fetchPaises();
    }).catch(error => {
        showAlert('error', 'Error al guardar el país');
    });
...
const formTitle = computed(() => {
    return editedIndex.value === -1 ? 'Nuevo Pais' : 'Editar Pais';
});

...
const getEmpleados: any = computed(() => {
    // return store.empleados;
    let empleados = store.empleados.map((empleado: any) => {
        return {
            ...empleado,
            employee_full_name:
                empleado.employee_last_name +
                (empleado.employee_second_last_name ? ' ' + empleado.employee_second_last_name : '') +
                ' ' +
                empleado.employee_name
        };
    });

    return empleados.sort((a: any, b: any) => {
        const nameA = a.employee_full_name.toUpperCase(); // ignore upper and lowercase
        const nameB = b.employee_full_name.toUpperCase(); // ignore upper and lowercase
    ...
    return store.empleados.map((empleado: any) => {
        return {
            ...empleado,
            employee_full_name:
                empleado.employee_last_name +
...
const headers: any = ref([
    { title: 'Nombre', align: 'start', key: 'employee_full_name' },
...
function deleteItem(item: any) {
    if (confirm('Are you sure you want to delete this item?')) {
        Object.assign(store.empleado, item);
        console.log(item);
...
function save() {
    Object.assign(store.empleado, editedItem.value);

    console.log(editedItem.value);

    let response;
    if (store.empleado.id) response = store.update();
...
function goTo(item: any) {
    // console.log(item);
    router.push(`/empleado/profile/${item.id}`);
}

...
onMounted(() => {
    storeDashboard.fetchAcumuladoRubro();
});

const categorias: any = computed(() => {
    return storeDashboard.acumulado_rubro.map((item: any, i) => {
        return item.nombre;
    });
});

const series: any = computed(() => {
    // console.log(storeDashboard.acumulado_rubro);

    let series_: any = [];
    if (storeDashboard.acumulado_rubro.length) {
        let tmp: any = [];
        tmp = (storeDashboard.acumulado_rubro[0] as never)['items'];

        series_ = tmp.map((item: any) => {
            return { name: item.nombre, data: [] };
        });

        storeDashboard.acumulado_rubro.map((pitem: any) => {
            pitem.items.map((item: any) => {
                for (var i in series_) {
                    if (series_[i].name == item.nombre) {
                        series_[i].data.push(item.total);
                    }
                }
            });
        });
    ...
    <v-card elevation="10">
        <v-card-text>
            <div class="d-md-flex align-center">
                <div>
                    <h3 class="text-h5 title mb-1">Acumulado por Rubros</h3>
